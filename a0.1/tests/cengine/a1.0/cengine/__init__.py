...
def draw():
    ...
    #get the canvas origin values to determine where to start x/y draw (origin can change for canvas translation)    
    xOrigin = cengine.canvas.xOrigin
    yOrigin = cengine.canvas.yOrigin
    
    #get terminal width/height
    winWidth = cengine.graphics.getWidth()
    winHeight = cengine.graphics.getHeight()     
    
    if xOrigin < 0: 
        xStart = (xOrigin) * (-1)   
        xStop = winWidth
        if len(buffer) + xStart > winWidth: xStop = winWidth
        elif len(buffer) + xStart <= winWidth: xStop = len(buffer) + xStart
        xOff = xStart
    elif xOrigin >= 0:
        xStart = 0
        xStop = 0
        if len(buffer) - xOrigin > winWidth: xStop = winWidth
        elif len(buffer) - xOrigin <= winWidth: xStop = len(buffer) - xOrigin
        xOff = (xOrigin) * (-1)
        
    if yOrigin < 0: 
        yStart = (yOrigin) * (-1)   
        yStop = winHeight
        if len(buffer[0]) + yStart > winHeight: yStop = winHeight
        elif len(buffer[0]) + yStart <= winHeight: yStop = len(buffer[0]) + yStart
        yOff = yStart
    elif yOrigin >= 0:
        yStart = 0
        yStop = 0
        if len(buffer[0]) - yOrigin > winHeight: yStop = winHeight
        elif len(buffer[0]) - yOrigin <= winHeight: yStop = len(buffer[0]) - yOrigin
        yOff = (yOrigin) * (-1)
        
    #loop through and draw the canvas buffer
    for a in range(xStart, xStop):
        for b in range(yStart, yStop):
            cengine.graphics.addstr(a, b, buffer[a - xOff][b - yOff])
